...
import { allPostData, boostedPost } from "../../Data/Data.js";
import moment from "moment/moment.js";

const BoostedPostDataTable = ({ searchQuery, title = 'Edit a Pharmacy',  setRows, loading }) => {
    ...
    const [boostedPostData,setBoostedPost]=useState();
    useEffect(()=>{
        const filteredBoostedPost = searchQuery != null && searchQuery != "" 
        ? boostedPost.filter((element) => {
        ...
        : boostedPost;
        setBoostedPost(filteredBoostedPost)
    },[searchQuery])
            ...
            <div className="row  ml-0 mx-2 " style={{ overflowX: "hidden" }}>
                ...
                >
                    <Table aria-label="simple table">
                        ...
                        >
                            {!getHospital?.isLoading ? boostedPostData?.map((post, index) => (
                                <TableRow
                                    ...
                                    <TableCell align="left">
                                        <CardHeader
                                            ...
                                            avatar={
                                                ...
                                                >
                                                    <Avatar alt="Hospital Pic" src={`${ post?.profile}`} onClick={() => handleImageClick(post?.profile)} />
                                                </Box>
                                            }
                                            title={post?.author}
                                        />
                                    </TableCell>
                                    <TableCell align="center">{post?.university}</TableCell>
                                    <TableCell align="center">{post?.comments}</TableCell>
                                    <TableCell align="center">{post?.likes}</TableCell>
                                    <TableCell align="center"> 
                                    <Tooltip placement="topRight" title={post?.content} >
                                    {post?.content?.length > 22 ? post?.content?.slice(0, 22) + '...' : post?.content}
                                    </Tooltip>
                                    </TableCell>
                                    <TableCell align="center">{moment(post?.timestamp).format('DD-MM-YYYY HH:MM')}</TableCell>
                                    <TableCell align="center">{`${post?.paymentDetails?.totalAmount}`}</TableCell>
                                    <TableCell >
                                        <img className='cursor-pointer' onClick={() => {
                                            setDeleteModal(true)
                                            setDeleteState(post?.post_id)
                                        }} src={DeleteIcon} />
                                    </TableCell>
                                </TableRow>
                            )) :
...
export default BoostedPostDataTable;