...
console.log("selectedImageeeee", selectedImage)
    const handleImageUpload = (event) => {
        const imageFile = event.target.files[0];
        setSelectedImage(imageFile);
        // setSelectedImageParent(imageFile)
    ...
    return (
        <div>
            ...
            />
            {selectedImage ? <div className='upload-profile-img-final cursor-pointer' style={{ width: "80px", heigth: '80px' }} onClick={handleButtonClick}>
                <img  src={ !role_IdState ?  URL.createObjectURL(selectedImage) :   process.env.REACT_APP_IMAGE_URL +
                          selectedImage} alt="Uploaded"  />
            </div> :
                <div className='upload-profile-img cursor-pointer' style={{ width: "80px", heigth: '80px' }} onClick={handleButtonClick} >
